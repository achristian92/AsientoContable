...
export default {
    ...
    methods: {
        downloadTemplate() {
            window.location =  `${this.baseUrl}api/customer/${this.currentCustomerID}/template-payroll`
        },
        submitImport() {
            this.isLoading = true
            ...
            axios.post(`${this.baseUrl}api/customer/${this.currentCustomerID}/payroll-import`, data,{
                headers: { 'content-type': 'multipart/form-data' }
            }).then(res => {
                this.isLoading = false

                if (res.data.is_correct === false) {
                    Vue.$toast.error(res.data.msg)
                    return '';
                }

                if (res.status === 201) {
                    $('#importModalPayroll').modal('hide')
                    this.remove()
