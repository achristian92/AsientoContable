        ...
        <div class="app-sidebar-menu" tabindex="1" style="overflow: hidden; outline: none;">
            <div class="list-group list-group-flush">
                @foreach($files as $file)
                    <a href="{{ route('admin.customers.payrolls.show',[$currentCustomer->id,$file->id]) }}"
                       class="list-group-item d-flex align-items-center {{ (int) request()->segment(5) === $file->id ? 'text-primary' : '' }}">
                        ...
                        {{ $file->name }}
                        @if ($file->isOpen)
                            <span><i class="fa fa-folder-open ml-2 text-success"></i></span>
                        @else
                            <span><i class="fa fa-lock ml-2 text-danger"></i></span>
                        @endif
                        <span class="small ml-auto">{{ $file->totalConcepts }}</span>
                    </a>
                @endforeach
            </div>
            @if ($files->isEmpty())
